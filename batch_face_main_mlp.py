...
import batch_image_process_mlp
import time
...
class video_process():
    ...
    def process(self,parent,video_path):
        mlp_list = []
        for directory, folders, files in os.walk('{}/'.format(self.args.mlp.split('/')[0])):
            for file in files:
                if len(file.split('.')) == 3:
                    mlp_list.append(file)
            break
        mlp_list.sort()
        #print(mlp_list)
        
        def one_model_process(self, parent, video_path, image_process):
...
#                if not os.path.exists(parent.result_path):
#                    os.makedirs(save_path)
                ...
                while ret and img is not None:
                    if frame_id % 10 == 0:
                        ...
                        t0 = time.time()
                        label, probability,coordinate, signal= image_process.main(img_input)
        ...
        for index, mlp in enumerate(mlp_list):
            parent.result_path = self.args.result_path + '_' + '{}k'.format(int(mlp.split('.')[1].split('-')[1])/1000)
            #self.args.result_path = self.args.result_path + '_' + '{}k'.format(int(mlp.split('.')[1].split('-')[1])/1000)
            self.args.mlp = 'face_model/{}'.format(mlp)
            print('index: ',index,parent.result_path,self.args.mlp)
            image_process = batch_image_process_mlp.image_process(self.args)
            one_model_process(self,parent, video_path, image_process)
...
def parse_arguments(argv):
    ...
    parse.add_argument('--facenet',type = str, help = 'extract feature in the image',default = 'face_model/face_565_2018_8_26.pb')
    parse.add_argument('--mlp',type = str, help = 'classifer through the mlp',default = 'face_model/ckpt')
    parse.add_argument('--face_classname_path',type = str, help = 'face classname',default = 'face_model/face_class_name_2018_8_26.txt')
    parse.add_argument('--result_path', type = str, help = 'processed the images saved',default = 'mlp_565_2018_8_26')
